...
sub isolateRe {
  ...
  if (! ref($pattern)) {
    $rc = ($string =~ m/$pattern/gp) ? 1 : 0;
  } else {
    $rc = ($string =~ $pattern) ? 1 : 0;
  }
...
# qr input type
#
ok (isolateRe('test', '\(tes\)t'), "'test' =~ /\(tes\)t/");
ok (isolateRe('test', [ 0, '\(tes\)t' ]), "'test' =~ [ 0, '\(tes\)t' ]");
ok (isolateRe('test', { syntax => 0, pattern => '\(tes\)t' }), "'test' =~ { syntax => 0, pattern => '\(tes\)t' }");
#
# Gnulib own test
#
ok (! isolateRe("\x{FF}\0\x{12}\x{A2}\x{AA}\x{C4}\x{B1},K\x{12}\x{C4}\x{B1}*\x{AC}K", { syntax =>
...
#
use Data::Dumper;
  print STDERR "ATTACH ME: $$\n";
  sleep(10);

ok (isolateRe("\x{1000}\x{103B}\x{103D}\x{1014}\x{103A}\x{102F}\x{1015}\x{103A}xy", '\([^x]\)\(x\)/'), "\"\\x{1000}\\x{103B}\\x{103D}\\x{1014}\\x{103A}\\x{102F}\\x{1015}\\x{103A}xy\" =~ qr/\([^x]\)\(x\)/p");
is ($1, "\x{103A}", "utf8 \$1");
is ($2, "x", "utf8 \$2");
is ($-[0], 7, "utf8 \$-[0]");
is ($+[0], 9, "utf8 \$-[0]");
